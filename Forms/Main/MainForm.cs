using Po.Devices;
using Po.Devices.BlueInOne;
using Po.Devices.Cozir;
using Po.Devices.Gas;
using Po.Devices.Pump;
using Po.Devices.Scale;
using Po.Devices.UVFlux;
...
namespace DeviceManagerGUI
{
    public partial class MainForm : Form
    {
		...
		private void MainMenu_Settings_Click(object sender, EventArgs e) => new SettingsForm(this, _dataCollector.IsRunning).ShowDialog();
		...
		private void UpdateDataCollectorButton() => Button_StartCollector.Enabled = Devices.Where(d => d.ConnectionState == ConnectionState.Connected).Count() > 0 || (_dataCollector?.IsRunning ?? false);

		private void Button_StartCollector_Click(object sender, EventArgs e) => TurnDataCollector();
		private void Button_OpenOutputDirectory_Click(object sender, EventArgs e) => Process.Start(Settings.Default.OutputDirectory);
		private void Button_OpenLaunchDirectory_Click(object sender, EventArgs e) => Process.Start(new DirectoryInfo(Assembly.GetEntryAssembly().Location).Parent.FullName);
